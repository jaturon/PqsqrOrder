...
(function ($, undefined) {
    ...
    $.widget("mobile.listviews", $.mobile.listview, {
        ...
        _beforeListviewRefresh: function () {
            ...
            if (this._editMode) {
                ...
                if (opts.itemIcon) {
                    $el.append($origDom.clone().find('li').wrapInner('<a onclick=\'slCallback($(this))\'></a>'));
                } else {
                    $el.append($origDom.clone().find('li').attr( "data-icon", "false").wrapInner('<a onclick=\'slCallback($(this))\'></a>'));
                }
            }
        ...
        _afterListviewRefresh: function () {
        	...
        	this.element.parents(":jqmData(role='collapsible')").collapsible("expand");
        ...
        _insertListItem: function (e) {
            ...
            if (this.options.editableType === 'simple') {
				...
				var oC = this._origDom[0].attributes["data-item-name"].nodeValue;
				var vtxt = inputTextString;
        ...
        _deleteListItem: function (e) {
            var $parentTarget = $(e.currentTarget).parent(),
				...
				var oC = $(e.currentTarget).parent().parent()[0].attributes["data-item-name"].nodeValue;
				var vid = $(e.currentTarget).parent()[0].value;

				keepDeletedID(oC, vid);

			// check if  data have been used in other place
				// alert worning
			 //if not delete ok 
